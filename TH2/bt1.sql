...
/*Câu 2: Tạo query cho xem kết quả thi của sinh viên, thông tin gồm các field: MaSV,
HoTen,TenHP, DiemLan1, KetQua, trong đó: field KetQua được xét như sau: nếu
DiemLan1>=5 thì đạt, ngược lại thì không đạt. */
SELECT        SinhVien.MaSV, SinhVien.HoDem, HocPhan.TenHP, KetQua.Diemlan1, case when KetQua.Diemlan1 >=5 then 'Dat' else 'Khong Dat' end KetQua
...
FROM(
SELECT        SinhVien.MaSV, SinhVien.HoDem, SinhVien.Ten, ROUND(AVG(case when isnull(DiemLan1,0) > isnull(DiemLan2,0) then isnull(DiemLan1,0) else isnull(DiemLan2,0) end),1) as DiemTB
FROM            SinhVien INNER JOIN
